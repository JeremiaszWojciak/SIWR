        ...
        for cur_loc in self.front:
            ...
            for num_pits in range(len(front_other) + 1):
                for cur_pits in combinations(front_other, num_pits):
                    ...
                    # probability of breeze givern known, query and frontiers
            ...
            P_q_giv_k_b_norm_comp = P_q_giv_k_b / (P_q_giv_k_b + P_nq_giv_k_b)
            # ------------------
    ...
    def prob_bf(self, query: Tuple[int, int], vis: Union[Sequence[Tuple[int, int]], Set[Tuple[int, int]]]):
        ...
        for num_pits in range(len(unknown) + 1):
            for cur_pits in combinations(unknown, num_pits):
                # print('cur_pits: ', cur_pits)
                # if query contains pit then add it to set of pits
                pits_q = set(cur_pits).union({query})
                pits_nq = set(cur_pits)
                # print('pits_q: ', pits_q)
                # print('pits_nq: ', pits_nq)

                # probability of breeze given known, query and unknown
